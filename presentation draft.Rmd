...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("Our Killer Plot: Relative Ride Destinations"),
   
   # Sidebar with a dropdown input for number of bins 
   sidebarLayout(
      sidebarPanel(
         selectInput(inputId = "start_stations", 
                     label = "Choose a Starting Station"),
      ),
...
server <- function(input, output, session) {
  ...
  bike_dts$circle_x <- cos(bike_dta$bearings)
  ...
  observe({
    updateSelectInput(session, "start_stations", choices = sales$SalesRep)
  })
