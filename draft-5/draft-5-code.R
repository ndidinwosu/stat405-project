...
dbListTables(dcon)
table2 <- read.csv(paste0("C:/Users/Ndidi/Desktop/2022-23/fall/STAT 405/202208-capitalbikeshare-tripdata/202208-capitalbikeshare-tripdata.csv"))
...
ggplot(data = station_travel[-1,], aes(x = start_station_name, y = num_trips)) +
  ...
  theme_minimal()
```
This graph shows the number of trips that start from a select number of stations. As the graph shows, there seems to be a high concentration of bike rides that start from Jefferson Dr & 14th St SW, which this station having the most bike rides by a large margin. Meanwhile, the least popular starting station seems to be Roosevelt Island. Jefferson Dr & 14th St SW may be a popular station where there is a higher concentration of bike riders, so investing in more bikes at this station, or creating more bike stations near this area may be a profitable move for this Bike Share company, while investing less in places near Roosevelt Island may not be.
